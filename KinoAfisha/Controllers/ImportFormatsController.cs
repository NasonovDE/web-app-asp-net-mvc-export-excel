...
namespace KinoAfisha.Controllers
{
    public class ImportFormatsController : Controller
    {
        ...
        {
            return File("~/Content/Files/ImportDescriptionsExample.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "ImportDescriptionsExample.xlsx");
        }
            ...
            foreach (var row in rows)
            {
                try
                {
                    var data = new ImportDisciplineData()
                    {
                        ...
                        DescriptionCinemaCorporation = ConvertToString(row.Cell("B").GetValue<string>().Trim()),
                        DescriptionAllFilms = ConvertToString(row.Cell("C").GetValue<string>().Trim()),
                        DescriptionAllActors = ConvertToString(row.Cell("D").GetValue<string>().Trim()),
        ...
        private void ApplyImported(List<ImportDisciplineData> data)
        {
            ...
            foreach (var value in data)
            {
                ...
                {
                    Name = value.Name,
                    DescriptionCinemaCorporation = value.DescriptionCinemaCorporation,
                    DescriptionAllFilms = value.DescriptionAllFilms,
                    DescriptionAllActors = value.DescriptionAllActors,
